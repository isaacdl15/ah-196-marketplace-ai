  ...
  return (
    <div style={{ border: '1px solid #E7E5E4', borderRadius: '12px', overflow: 'hidden' }}>
      <div style={{
        ...
        {template.demo_url && (
          <a href={template.demo_url} target="_blank" rel="noopener noreferrer" style={{ color: '#57534E' }}>
            <Globe size={14} />
          </a>
        )}
      </div>
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          ...
          {template.is_free && (
            <span style={{ padding: '4px 12px', background: '#16A34A', color: '#FFFFFF', borderRadius: '6px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase' }}>
              Free
            </span>
          )}
        </div>
        ...
        <p style={{ fontSize: '16px', color: '#57534E', maxWidth: '480px', lineHeight: 1.6 }}>
...
export default function TemplateDetailClient({
  ...
  const handleBuy = async () => {
    if (!isLoggedIn) {
      window.location.href = `/auth/signup?redirect=/template/${template.slug}`;
      return;
    }
    setPurchasing(true);
    // Call checkout API
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF9' }}>
      ...
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E7E5E4', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
        <Link href="/browse" style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#57534E', textDecoration: 'none', fontSize: '13px', fontWeight: 500 }}>
          <ArrowLeft size={16} />
          Browse
        </Link>
        ...
        {!isLoggedIn && (
          <Link href="/auth/login" style={{ padding: '7px 16px', border: '1px solid #E7E5E4', borderRadius: '8px', fontSize: '13px', fontWeight: 600, color: '#57534E', textDecoration: 'none' }}>
            Sign in
          </Link>
        )}
      </div>
        ...
        <div>
          ...
          <div style={{ padding: '28px 0' }}>
              ...
              <div>
                {template.features && template.features.length > 0 && (
                  <div style={{ marginBottom: '32px' }}>
                    <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#1C1917', marginBottom: '16px' }}>
                      What&apos;s included
                    </h3>
                        ...
                        {seller.categories && seller.categories.length > 0 && (
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '8px' }}>
                            {seller.categories.slice(0, 4).map(cat => (
                              <span key={cat} style={{ padding: '2px 8px', background: '#EEF2FF', color: '#4F46E5', borderRadius: '6px', fontSize: '11px', fontWeight: 600 }}>
                                {cat}
                              </span>
                            ))}
                          </div>
            ...
            {activeTab === 'reviews' && (
              <div>
                {reviews.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '48px', color: '#A8A29E', fontSize: '14px' }}>
        ...
        <div>
          <div style={{
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ fontSize: '32px', fontWeight: 800, color: template.is_free ? '#16A34A' : '#1C1917', fontFamily: 'var(--font-family-display)' }}>
                {price}
              </div>
              {!template.is_free && (
                <p style={{ fontSize: '12px', color: '#A8A29E', marginTop: '2px' }}>One-time purchase, lifetime access</p>
              )}
            </div>
            ...
            <button
              ...
              style={{
                width: '100%',
                padding: '14px 16px',
                background: purchasing ? '#A8A29E' : '#4F46E5',
                color: '#FFFFFF',
                border: 'none',
                borderRadius: '10px',
                fontSize: '15px',
              ...
              onMouseEnter={e => { if (!purchasing) (e.currentTarget.style.background = '#3730A3'); }}
              onMouseLeave={e => { if (!purchasing) (e.currentTarget.style.background = '#4F46E5'); }}
            >
              <ShoppingCart size={16} />
              {purchasing ? 'Processing...' : template.is_free ? 'Get for Free' : `Buy for ${price}`}
            </button>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {template.demo_url && (
                <a href={template.demo_url} target="_blank" rel="noopener noreferrer"
                  style={{
                    flex: 1, padding: '9px', border: '1px solid #E7E5E4', borderRadius: '8px',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                    fontSize: '12px', fontWeight: 500, color: '#57534E', textDecoration: 'none',
                  }}>
